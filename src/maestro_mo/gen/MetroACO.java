...
public class MetroACO implements Generator 
{
	...
	public ArrayList<SolutionRoot> generateSolutions(Population population,	int number)
	{
		...
		while (roots.size() < number)
		{
			...
			{
				if (acoCount == 0)
					roots.addAll(metropolisGenerate(discVarCount, contVarCount, varCount, solList, 
														contDist, groupSize));
				else
					roots.addAll(metroACOGenerate(discVarCount, contVarCount, varCount, solList, 
									contDist, acoCount, groupSize, discVals, contVals, acoDims));
			}
			else
	 ...
	 */
	private void assignWeights(ArrayList<SolutionWrapper> solutions)
	{
		// Obtain maximum rank
		int maxRank			= 0;
		...
		for (SolutionWrapper solution : solutions)
		{
			...
			solution.setValue(SolutionWrapper.WEIGHT, weight);
		}
	}
	...
	private ArrayList<DiscProbDist> createDiscreteDistributions(int discVarCount,
						ArrayList<SolutionWrapper> solutions) 
	{
		...
		for (SolutionWrapper solution : solutions)
		{
			...
			double weight				= solution.getValue(SolutionWrapper.WEIGHT);
			for (int i = 0; i < discVarCount; i++)
				values.get(i).addValue(solVal.get(i), weight);
		}
		ArrayList<DiscProbDist> discDists = new ArrayList<>();
		for (int i = 0; i < discVarCount; i++)
			discDists.add(new DiscProbDist(values.get(i)));
		return discDists;
	}
							...
							ArrayList<SolutionWrapper> solutions)
	{
		...
		for (SolutionWrapper solution : solutions)
		{
			...
			double weight				= solution.getValue(SolutionWrapper.WEIGHT);
			Sample sample				= new Sample(weight, values);
	...
	private ArrayList<SolutionRoot> metropolisGenerate(int discVarCount, int contVarCount,
			int varCount, ArrayList<SolutionWrapper> solList, MultiVarKernelDensity contDist, 
			int groupSize) 
	{
		ArrayList<SolutionRoot> roots	= new ArrayList<>();
		...
		for (int j = 0; j < solList.size(); j++)
			selector.addValue(j, solList.get(j).getValue(SolutionWrapper.WEIGHT));
		ArrayList<Double> solutionIDs	= selector.sampleMultiple(groupSize, true, true);
		
		// Generate values
		for (int j = 0; j < groupSize; j++)
		{
			SolutionWrapper solution		= solList.get((int)(double)solutionIDs.get(j));
			...
			if (contVarCount > 0)
			{
				...
				double[] bwDiag				= contDist.getDiagBandwidth();
				MultiVarNormal kernel		= new MultiVarNormal(contSolA, bwDiag);
	...
	private ArrayList<SolutionRoot> metroACOGenerate(int discVarCount, int contVarCount,
			int varCount, ArrayList<SolutionWrapper> solList, MultiVarKernelDensity contDist, 
			int acoCount, int groupSize, ArrayList<Integer> discVals, ArrayList<Double> contVals, 
			ArrayList<Integer> acoDims)
	{		
		...
		for (int j = 0; j < solList.size(); j++)
		{
			SolutionWrapper solution	= solList.get(j);
			double weight				= 1.0;
			if (discVarCount > 0)
			{
			...
			weight						*= solution.getValue(SolutionWrapper.WEIGHT);
