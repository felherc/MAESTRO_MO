 ...
 * all the objectives. New solutions are then sampled in the direction of a random linear 
...
public class GD implements Generator
{
	 ...
	 */
	public final static double DEF_POWER_VICINITY_WEIGHT = 1.0;
	
	/**
	 * Default value for {@link #stepSize}
	 */
	public final static double DEF_STEP_SIZE = 0.1;
	
	/**
	 * Default value for {@link #amplitude}
	 */
	public final static double DEF_AMPLITUDE = 0.5;
	
	/**
	 * Default value for {@link #uniformProb}
	 */
	public final static double DEF_UNIFORM_PROB = 0.05;
	
	/**
	 * Error message: no variables defined
	 */
	public final static String ERR_NO_VARIABLES = "No decision variables defined";
	
	/**
	 * Error message: no objective functions defined
	 */
	public final static String ERR_NO_OBJECTIVES = "No objective functions defined";
	...
	/**
	 * The list of objectives of the problem
	 */
														...
														int number)
	{
		...
		if (discVarCount + contVarCount == 0)
			throw new IllegalArgumentException(ERR_NO_VARIABLES);
		if (objectiveCount == 0)
			throw new IllegalArgumentException(ERR_NO_OBJECTIVES);
		...
		ArrayList<SolutionWrapper> solList	= population.getAllSolutions();
		for (SolutionWrapper solution : solList)
		{
		...
		if (validScalarCount > solList.size() - 1)
			return null;
		
		// Compute statistics for the objectives
		
		ArrayList<ContSeries> objectStats	= new ArrayList<>();
		ArrayList<Integer> customObjIDs		= new ArrayList<>();
		ArrayList<Integer> fitnessObjIDs	= new ArrayList<>();
		for (int j = 0; j < objectiveCount; j++)
		{
			if (objectives.get(j).isCustom())
				customObjIDs.add(j);
			else
			{
				fitnessObjIDs.add(j);
				objectStats.add(new ContSeries(false));
			}
		}
		for (SolutionWrapper solution : solList)
			for (int j = 0; j < fitnessObjIDs.size(); j++)
				objectStats.get(j).addValue(solution.getFitness(fitnessObjIDs.get(j)));
		ArrayList<TreeSet<SolutionWrapper>> ordered = new ArrayList<>();
		for (int j = 0; j < customObjIDs.size(); j++)
		{
			TreeSet<SolutionWrapper> order = new TreeSet<>(objectives.get(customObjIDs.get(j)));
			order.addAll(solList);
			ordered.add(order);
		}
		
		// Create normalized solutions
		ArrayList<NormSolution> normSolList	= createNormalizedSolutions(contVarCount, 
				discScalarIDs, scalarStats, zeroVar, solList, objectiveCount, objectStats, 
				fitnessObjIDs, ordered);
		
		// Select bases for vicinities
		TreeSet<Integer> baseIDs			= getVicinityBases(solList, validScalarCount);
		...
		for (int i = 0; i < baseIDs.size(); i++)
		{
			...
			while (vicinity.size() < validScalarCount + 1)
			{
				NormSolution candidate		= normSolList.get(iter.next().getIndex());
		...
		for (int v = 0; v < vicinities.size(); v++)
		{
			...
			selector.addValue(v, Math.pow(vicinity.getGradMagnitude(), powerVicinityWeight));
			toGenerate[v]					= 0;
		}
		...
		ArrayList<SolutionRoot> roots		= unnormalizeSolutions(discVarCount, contVarCount, 
										discScalarIDs, scalarStats, zeroVar, solList, solutions);
	 ...
	 * @param objectStats		The statistics of the objectives
	 * @param fitnessObjIDs		The indexes of the non-custom objectives
	 * @param ordered			The list of sets where the solutions are ranked for each custom
	 * 							objective
	...
	private ArrayList<NormSolution> createNormalizedSolutions(int contVarCount,
			...
			HashSet<Integer> zeroVar, ArrayList<SolutionWrapper> solList, int objectiveCount, 
			ArrayList<ContSeries> objectStats, ArrayList<Integer> fitnessObjIDs,
			ArrayList<TreeSet<SolutionWrapper>> ordered)
	{
		...
		for (SolutionWrapper solution : solList)
		{
			...
			double[] normFitness			= new double[objectiveCount];
			int fitObIDsIndex				= 0;
			int cusObIDsIndex				= 0;
			for (int j = 0; j < objectiveCount; j++)
			{
				if (j == fitnessObjIDs.get(fitObIDsIndex))
				{
					double original			= solution.getFitness(j);
					ContSeries stats		= objectStats.get(fitObIDsIndex);
					double stDev			= stats.getStDev();
					double temp				= stDev == 0.0 ? 0.0 : (original - stats.getMean())
																	/stDev;
					boolean maximize		= objectives.get(j).isMaximization();
					normFitness[j]			= maximize ? temp : -temp;
					fitObIDsIndex++;
				}
				else
				{
					TreeSet<SolutionWrapper> order = ordered.get(cusObIDsIndex);
					double rank				= order.tailSet(solution).size();
					Uniform dist			= new Uniform(0, order.size() - 1);
					normFitness[j]			= (rank - dist.getMean())/dist.getStDev();
					cusObIDsIndex++;
				}
			}
	 ...
	 * @return The set of 
	 */
	private TreeSet<Integer> getVicinityBases(ArrayList<SolutionWrapper> solList, int scalarCount) 
	{		
		...
		int maxRank					= 0;
		for (SolutionWrapper solution : solList)
			maxRank					= Math.max(solution.getRank(), maxRank);
		...
		// Assign weights
		ContSeries selector					= new ContSeries(true);
		double stDev				= q*maxRank;
		for (int i = 0; i < solList.size(); i++)
		{
			SolutionWrapper solution	= solList.get(i);
			double weight			= Normal.computepdf(1.0, stDev, solution.getRank());
	...
	private ArrayList<SolutionRoot> unnormalizeSolutions(int discVarCount, int contVarCount,
			ArrayList<Integer> discScalarIDs, ArrayList<ContSeries> scalarStats,
			HashSet<Integer> zeroVar, ArrayList<SolutionWrapper> solList, 
			ArrayList<NormSolution> solutions)
	{
		...
		for (NormSolution normSolution : solutions)
		{
			...
			SolutionWrapper base			= solList.get(normSolution.getBaseIndex());
			int normIndex					= 0;
			for (int v = 0; v < scalarStats.size(); v++)
			{
